...
def get_workers_without_scales(subsidiarie_id):
...
def get_workers_with_less_than_ideal_days_off(subsidiarie_id):
    with Session(engine) as session:
        today = datetime.now()
...
def get_open_jobs(subsidiarie_id):
    with Session(engine) as session:
        jobs = session.exec(
...
async def get_subsidiary_notifications(id: int):
    workers_without_scales = get_workers_without_scales(id)

    open_jobs = get_open_jobs(id)

    workers_with_less_than_ideal_days_off = get_workers_with_less_than_ideal_days_off(
        id
    )
