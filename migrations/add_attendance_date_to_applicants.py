...
def add_attendance_date_to_applicants():
    ...
    with Session(engine) as session:
        if db_dialetics == "sqlite":
            ...
            if "attendance_date" not in columns:
                session.exec(
                    text("ALTER TABLE applicants ADD COLUMN attendance_date TEXT")
                )
            ...
            result = session.exec(
                text(
                    ...
                    WHERE table_name = 'applicants' AND column_name = 'attendance_date'
                    """
                )
            )
